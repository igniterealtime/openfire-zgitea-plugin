...
public class GiteaLoginService extends AbstractLifeCycle implements LoginService
{
    private static final Logger Log = LoggerFactory.getLogger(GiteaLoginService.class);
    private IdentityService _identityService = new DefaultIdentityService();
    private String _name;
    private UserManager userManager = XMPPServer.getInstance().getUserManager();

    protected GiteaLoginService()
    {

    }

    public GiteaLoginService(String name)
    {
        setName(name);
    }
    ...
    public UserIdentity login(String userName, Object credential, ServletRequest request)
    {
        ...
        try {
            String token = userName + ":" + credential;
            ...
            get.setHeader("authorization", "Basic " + encodedString);
            ...
            Log.error( "access denied, bad password " + userName, e);
            return null;
        }
        ...
        subject.getPrincipals().add(new RolePrincipal("gitea"));
        subject.setReadOnly();

        identity = _identityService.newUserIdentity(subject, userPrincipal, new String[] {"gitea"});
        return identity;
    }
